...
The tcpdump analysis log reveals that UDP port 53 is unreachable when attempting to reach the client website yummyrecipesforme.com. This is based on the results of the network analysis which show that the ICMP protocol was used to respond with an error message: "udp port 53 unreachable". Port 53 is associated with DNS protocol traffic so this may indicate a problem with the DNS server, or a DoS attack.

## * Explain your analysis of the data and provide at least one cause of the incident
The incident occurred at 1:24PM when customers reported that they were unable to access the website yummerecipesforme.com. I responded by attempting to access the website as well with the same result. When analyzing the data packets being sent and received using tcpdump, it was discovered that ICMP packets containing the message "udp port 53 unreachable" were sent back to the DNS server indicating unsuccessful communication. This incident was likely caused by a down DNS server, or a successful DoS attack.
